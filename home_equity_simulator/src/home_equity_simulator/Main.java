...
public class Main {
	...
	public static void run(Scanner scanner) {
	
	 {
	    ...
	    int fileSelectionIndex = promptForInt("Select dataset index (1-" + availableFiles.size() + "): ", 1, availableFiles.size(),scanner);
	    ...
	    while (running) {
	        ...
	        switch (choice) {
	            case "1":
	                User singleUser = promptForUserProfile("",scanner);
	            ...
	            case "2":
	                User standardUser = promptForUserProfile("",scanner);
	            ...
	            case "3":
	                ...
	                User user1 = promptForUserProfile("First ",scanner);
	                System.out.println("\n--- CONFIGURING PROFILE 2 ---");
	                User user2 = promptForUserProfile("Second ",scanner);
	...
	public static User promptForUserProfile(String prefix,Scanner scanner) {
        System.out.print("Enter " + prefix + "Applicant Name: ");
        String name = scanner.nextLine().trim();

        double income = promptForDouble("Enter Gross Annual Income: $",scanner);
        double savings = promptForDouble("Enter Capital Liquid Savings (Assets): $",scanner);
        int creditScore = promptForInt("Enter Credit Score (300-850): ", 300, 850,scanner);

        return new User(name, income, savings, creditScore);
    }

    public static double promptForDouble(String promptText,Scanner scanner) {
        while (true) {
            try {
    ...
    public static int promptForInt(String promptText, int min, int max,Scanner scanner) {
        while (true) {
            try {
                System.out.print(promptText);
